...
# ──────────────────── CONFIGURATION ────────────────────
PROJECT_ID = "heatscape-app"       # ← your GCP project
...
def init_ee():
    """Initialise Earth Engine explicitly using the project ID."""
    try:
        ee.Initialize(project=PROJECT_ID)
        print(f"✓ Connected to Earth Engine (project: {PROJECT_ID})")
    except Exception as e:
        print("⏳ Initial auth required. Please follow the link below:")
        # Force a headless/localhost authentication flow that doesn't rely on the broken notebook page
        try:
            ...
            print("Try running: earthengine authenticate --auth_mode=localhost")
            raise ex
